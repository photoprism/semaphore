...
echo "Starting 'photoprism/semaphore-$1' multi-arch build based on deployment/docker/${1/-//}$4/Dockerfile..."
echo "Build Arch: $2"
...
if [[ $1 ]] && [[ $2 ]] && [[ -z $3 || $3 == "preview" ]]; then
    ...
    docker buildx build \
      ...
      -f deployment/docker/${1/-//}$4/Dockerfile \
      -t photoprism/semaphore-$1:preview $5 \
      --push .
elif [[ $3 =~ $NUMERIC ]]; then
    ...
    docker buildx build \
      ...
      -f deployment/docker/${1/-//}$4/Dockerfile \
      -t photoprism/semaphore-$1:latest \
      -t photoprism/semaphore-$1:$3 $5 \
      --push .
    ...
    docker buildx build \
      ...
      -f deployment/docker/${1/-//}$4/Dockerfile \
      -t photoprism/semaphore-$1:$3 \
      -t photoprism/semaphore-$1:$BUILD_DATE-$3 $5 \
      --push .
else
    echo "Build Tags: $3"
    ...
    docker buildx build \
      ...
      --no-cache \
      --build-arg BUILD_TAG=$BUILD_DATE \
      -f deployment/docker/${1/-//}/Dockerfile \
      -t photoprism/semaphore-$1:$3 \
      --push .
fi
